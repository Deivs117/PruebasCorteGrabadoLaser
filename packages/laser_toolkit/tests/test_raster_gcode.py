...
def test_matriz_vacia_no_genera_gcode():
    assert gcode_grabado_raster([], 10, 10, 0, 0, 500, 0, 100, MachineConfig()) == []


def test_termina_apagando_el_laser():
    machine = MachineConfig()
    matriz = [[0.5, 0.5], [0.5, 0.5]]
    lineas = gcode_grabado_raster(matriz, 10, 10, 0, 0, 500, 0, 100, machine)
    assert lineas[-1] == "M5"
...
def test_una_fila_por_muestra_vertical():
    ...
    lineas = gcode_grabado_raster(matriz, 10, 6, 0, 0, 500, 0, 100, machine)
...
def test_pixel_negro_llega_al_techo_de_potencia():
    ...
    lineas = gcode_grabado_raster(matriz, 10, 10, 0, 0, 500, 0, 80, machine)
    # El techo (potencia_alta_pct=80% de laser_max_s=1000) debe aparecer.
...
def test_interpolacion_lineal_entre_potencia_baja_y_alta():
    """Interpolacion real (issue #95): intensidad 0.0 -> S de
    potencia_baja_pct, 1.0 -> S de potencia_alta_pct, 0.5 -> el punto medio
    exacto -- ya no una escala 0%-potencia_max_pct."""
    machine = MachineConfig(laser_max_s=1000)
    matriz = [[0.0, 0.5, 1.0]]
    lineas = gcode_grabado_raster(matriz, 10, 10, 0, 0, 500, 20, 80, machine)
    # baja=20% de 1000 = S200, alta=80% de 1000 = S800, medio=50% = S500.
    # baja=20% de 1000 = S200: el pixel mas claro (intensidad 0.0) nunca
    # apaga el laser del todo -- a diferencia del mapeo anterior, donde
    # intensidad 0.0 mapeaba siempre a S0.
    assert any(linea.endswith("S200") for linea in lineas)
    assert any(linea.endswith("S500") for linea in lineas)
    assert any(linea.endswith("S800") for linea in lineas)
...
def test_offset_desplaza_todo_el_barrido():
    machine = MachineConfig()
    matriz = [[1.0]]
    sin_offset = gcode_grabado_raster(matriz, 10, 10, 0, 0, 500, 0, 100, machine)
    con_offset = gcode_grabado_raster(matriz, 10, 10, 50, 30, 500, 0, 100, machine)
...
def test_rotacion_90_grados_intercambia_ancho_y_alto_del_recorrido():
    ...
    sin_rotar = gcode_grabado_raster(matriz, 4, 2, 0, 0, 500, 0, 100, machine)
    rotado = gcode_grabado_raster(matriz, 4, 2, 0, 0, 500, 0, 100, machine, angulo_rad=math.pi / 2)
